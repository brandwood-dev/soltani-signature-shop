...
export const Route = createFileRoute("/checkout")({
  ...
  head: () => ({
    meta: seoMeta({
      title: "Commande — Soltani Signature",
      description: "Finalisez votre commande Soltani Signature.",
      path: "/checkout",
      noindex: true,
    }),
...
function CheckoutPage() {
  ...
  const [quickLines, setQuickLines] = useState(() =>
    search.quick === "1" ? readQuickCheckoutLines() : [],
  );
  ...
  const subtotal = isQuickCheckout
    ? quickLines.reduce((sum, line) => sum + line.price * line.qty, 0)
    : cartSubtotal;
            ...
            {step === 2 && (
              <div className="space-y-5">
                ...
                <div className="rounded-sm border border-gold/30 bg-gold/5 p-4 text-sm">
                  {shipping === 0
                    ? "Livraison offerte."
                    : `${SHIPPING_LABEL} — tarif fixe ${settings.shippingFee} DT.`}
                </div>
              </div>
            )}
          ...
          <aside className="bg-card border border-border rounded-sm p-6 h-fit">
            ...
            <dl className="space-y-2 text-sm py-4 border-b border-border">
              <div className="flex justify-between">
                ...
                <dd className="tabular-nums">{subtotal} DT</dd>
              </div>
              <div className="flex justify-between">
...
function Field({ label, children }: { label: string; children: ReactNode }) {
  return (
    <label className="block">
      <span className="block text-[11px] uppercase tracking-widest text-foreground/60 mb-2">
        {label}
      </span>
      {children}
    </label>
